...
st.write("Upload Summary of Benefits PDFs for side-by-side comparison.")
...
if uploaded_files:
    ...
    for file in uploaded_files:
        with pdfplumber.open(file) as pdf:
            ...
            plan_info["Plan Name"] = file.name.replace(".pdf", "")
            results.append(plan_info)

    df = pd.DataFrame(results).set_index("Plan Name")
    st.subheader("📊 Plan Comparison")
    st.dataframe(df)
    st.download_button("Download as CSV", df.to_csv().encode("utf-8"), "plan_comparison.csv", "text/csv")
